...
while(cap.isOpened()):
    ...
    if len(contours) > 0:
        ...
        for i in range(0, len(contours)):
            ...
            cv2.putText(img, '(' + str(cx) + ',' + str(cy) + ')', (cx, cy + 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255),
                        1, cv2.LINE_AA)

        # Show Count Summary
        cv2.putText(img, 'Circle = ' + str(circleCount) + ' Triangle = ' + str(triangleCount) + ' Square = ' + str(
            squareCount) + ' Rectangle = ' + str(rectangleCount), (np.size(img, 1) - 450, np.size(img, 0) - 20),
